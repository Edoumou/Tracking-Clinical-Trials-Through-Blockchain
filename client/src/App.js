    ...
    if (!this.state.web3) {
      ...
      return (
        <div className="App ui container">
          <AuthorityAdmin contract={contract} role={role} account={account} />
        </div>
      );
    } else if (role === "AUTHORITY") {
      return (
        <div className="App ui container">
          <Authority contract={contract} role={role} account={account} />
        </div>
      );
    } else if (role === "PROMOTER ADMIN") {
      return (
        <div className="App ui container">
          <PromoterAdmin contract={contract} role={role} account={account} />
        </div>
      );
    } else if (role === "PROMOTER") {
      return (
        <div className="App ui container">
          <Promoter contract={contract} role={role} account={account} />
        </div>
      );
